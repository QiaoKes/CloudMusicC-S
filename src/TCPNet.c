...
int InitNetWork()
{
	...
	if((pool = (Pool_create(200,10,50))) == NULL)
		err_str("Create Thread_Pool Failed:",-1);

	Producer_add(pool,EPOLL_Jobs,(void*)pool);
	return TRUE;
}

void Addfd(int fd,int enable_et/*是否为边缘触发*/)
{
...
void * EPOLL_Jobs(void * arg)
{
	...
	while(1)
	{
		//阻塞-1监听socket
		printf("%d\n",i++);
		if((ready = epoll_wait(epfd,epollarr,_DEF_EPOLLSIZE,-1)) == -1)
			err_str("Epoll Call Failed:",-1);//出错
		Epoll_Deal(ready,pool);
		bzero(epollarr,sizeof(epollarr));
	}
}

void Epoll_Deal(int ready,pool_t *pool)
{
	int i = 0;
	for(i=0;i<ready;i++)
	{
		...
		if(sockfd == fd)   //客户端建立链接
			Producer_add(pool,Accept_Deal,NULL);
		else if(epollarr[i].events & EPOLLIN)
		{
			Deletefd(fd);
			Producer_add(pool,Info_Recv,(void*)fd);
		}
	}	
}
...
void *Accept_Deal(void *arg)
{
	...
	if((clientfd = accept(sockfd,(struct sockaddr*)&clientaddr,(socklen_t*)&clientsize)) == -1)
	{
...
int SendData(int clientfd,char* szbuf,int nlen)
{
